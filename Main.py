...
#   ----------------------------------------------------------------------------------------------------------------
def Para_Uplims_Lolims(Data, Para, Deviation):
    ParaLen = len(Para)
    Sign = ['+', '-']
    ...
    for Iter in product(range(2), repeat=ParaLen):
        ...
        i += 1
    X = random.uniform(Data[0, 0], Data[-1, 0], 100)
    for i in range(2**ParaLen):
        Para_Temp = Para_uplims_lolims[i, :]
        Yerror = sum(Function.xH(X, Para_Temp, EoS) - Function.xH(X, Para, EoS))
        if Yerror >= 0:
            Y_uplims[i] = Yerror
        if Yerror <= 0:
            Y_lolims[i] = Yerror
...
# rho = linspace(Data[0,0], Data[-1,0], 100)
# p = linspace(Data[0,1], Data[-1,1], 100)
# Data = transpose(vstack((rho,p)))
...
[Para_Uplims, Para_Lolims] = Para_Uplims_Lolims(Data, Para, Deviation)
End = time.clock()
TimeUsed = End - Start
