...
def GeneratePara(t, Para0, D, P):
    # [E_s^{-1},   C+ / C-,   C_s / C-,   gamma- - gamma+,   gamma- - gamma_s] -------------------------------------
...
def xH(Para, EoS):
    ...
    if rho[min_index] <= rho_crit:
        min_index += 1
    P_new = log10((Vs_crit * C.c) ** 2 * (10 ** rho[min_index:] - 10 ** rho_crit) + 10 ** P_crit)
    P[min_index:] = P_new
    ...
    for s in range(col):
        ...
        indexmax[s] = j
    return max(Msunt)  # Return the rho (density), r (radius), and M --------------------------------
...
def chi2(Para, Data, EoS):
    ...
    sigma = Data[1]
    Chi2 = sum((xH(Para, EoS) - xD)**2 / sigma**2)
    return Chi2
